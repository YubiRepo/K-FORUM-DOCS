# Accounting Module — Rules & Use Cases (v1.0)

Dokumentasi sistem rules modul Accounting KAI App. Accounting adalah **buku besar pencatatan uang masuk & keluar (in/out)** untuk operasional KAI, dipakai di **backoffice** oleh KAI Pusat (Superadmin) dan Admin Region.
...
**Accounting** adalah sistem pencatatan keuangan internal KAI untuk melacak arus kas (cashflow) operasional di tingkat Pusat maupun Region.

### Karakteristik:
- **Surface:** Backoffice only (tidak ada di mobile member)
- **Pengguna:** Superadmin (KAI Pusat) & Admin Region
- **Scope:** Per-region — mengikuti pola scope yang sama dengan Directory & Announcement
- **Fungsi inti:** Catat transaksi IN (pemasukan) / OUT (pengeluaran), kategorisasi, lihat saldo & laporan
- **Sumber data:** Manual (diketik admin) + future auto-record dari modul lain
...
Accounting mengisi gap ini: **satu tempat yang menjumlahkan semua uang masuk + keluar**, dengan scope per region dan kategorisasi yang rapi.
...
accounting_entry
  ...
  ├── region_id      : scope (null = Pusat, atau region tertentu)
...
**Saldo** = `SUM(amount_base WHERE direction=IN) − SUM(amount_base WHERE direction=OUT)` (filter per region/period sesuai kebutuhan).
...
- **`region_id`** → mewarisi pola scope region yang sudah ada di sistem.
...
| Aksi | Admin Region | Superadmin (Pusat) |
|------|:---:|:---:|
| Catat transaksi manual (IN/OUT) | ✅ region sendiri | ✅ semua region + Pusat |
| Lihat ledger | ✅ region sendiri | ✅ global + per region |
| Edit transaksi | ✅ miliknya, sebelum `verified` | ✅ semua |
| Void transaksi | ✅ miliknya, sebelum `verified` | ✅ semua |
| Verifikasi transaksi (jika diaktifkan) | ❌ | ✅ |
| Kelola kategori master | ❌ | ✅ |
| Lihat laporan & saldo | ✅ region sendiri | ✅ global |
| Export | ✅ region sendiri | ✅ semua |
| Konfigurasi setting accounting | ❌ | ✅ |

> Konsisten dengan pola yang ada: Admin Region menangani operasional region-nya, Superadmin memegang otoritas, master data, dan verifikasi (sama seperti Directory & Subscription).
...
verified → void            (hanya Superadmin, dengan alasan)
```

| Status | Deskripsi | Bisa diedit? |
|--------|-----------|:---:|
| `recorded` | Baru dicatat, default | ✅ (oleh pembuat/superadmin) |
| `verified` | Sudah diverifikasi Superadmin | ❌ (terkunci, hanya bisa di-void) |
| `void` | Dibatalkan — tetap tersimpan untuk audit, tidak masuk perhitungan saldo | ❌ |

> **Verifikasi bersifat OPSIONAL.** Diatur via setting `verification_required`. Jika `false`, entri langsung dianggap final saat `recorded` (tetap bisa diedit/void oleh yang berwenang). Jika `true`, entri perlu di-`verified` Superadmin sebelum masuk laporan resmi.

---

## 7. USE CASES

### Use Case 1: Admin Region catat pengeluaran event
```
Admin Region Jakarta → Accounting → "Catat Transaksi"
  ↓
Pilih: direction=OUT, category=EXP_EVENT, amount=Rp 5.000.000
       transaction_date=2026-06-10, attachment=foto kwitansi
  ↓
Submit → status=recorded (scope: region Jakarta)
  ↓
(Jika verification_required) Superadmin verifikasi → status=verified
```

### Use Case 2: Superadmin catat revenue subscription (manual, Phase 1)
```
...
### Use Case 3: Lihat laporan cashflow region
```
Admin Region → Accounting → Laporan
  ↓
Filter: period=Juni 2026, region=Jakarta
  ↓
Tampil: Total IN, Total OUT, Saldo, breakdown per kategori
```

### Use Case 4: Superadmin lihat laporan global
```
Superadmin → Accounting → Laporan Global
  ↓
Tampil: saldo seluruh region + Pusat, breakdown per region & kategori,
...
|---------|------|---------|-----------|
| `verification_required` | boolean | `false` | Jika true, entri perlu verifikasi Superadmin sebelum final |
| `default_currency` | string | `IDR` | Mata uang default saat input |
| `allow_region_admin_input` | boolean | `true` | Apakah Admin Region boleh catat transaksi |
...
- **Scope region** — Admin Region tidak bisa melihat/mengedit transaksi region lain. Superadmin lihat semua.
...
|----------------------|-------------------------------|
| **Auto-record revenue** dari Subscription & Ads | Field `source`+`source_ref` + endpoint internal `POST /accounting/entries` (dipanggil modul lain saat transaksi approve) |
...
| **Budgeting per region** | Tabel `accounting_budgets` (region + period + planned amount); entri dibandingkan vs budget |
...
### Roadmap
- **Phase 1:** Input manual (IN/OUT), kategori, scope region, laporan & saldo, multi-currency, export CSV. Field hook (`source`, `source_ref`, `external_txn_id`) disiapkan tapi belum dipakai.
...
| **Region** | Sumber `region_id` untuk scope setiap entri. |
| **Role/Permission** | Menentukan siapa boleh input/verifikasi (Admin Region vs Superadmin). |
...
Accounting = **buku besar in/out terpusat** untuk operasional KAI di backoffice, dengan scope per region mengikuti pola yang sudah ada. Setiap entri adalah event yang bisa berasal dari input manual maupun (nanti) auto-push modul lain. Desain `direction` + `source/source_ref` + `amount_base` membuatnya sederhana untuk dipakai sekarang tapi siap berkembang ke auto-record, payment gateway, budgeting, dan export — tanpa mengubah struktur inti.
